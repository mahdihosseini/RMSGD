...
for iteration_layer in range(input_rank_data.shape[1]):
    plt.subplot(np.ceil(np.sqrt(input_rank_data.shape[1])), np.ceil(np.sqrt(input_rank_data.shape[1])), iteration_layer+1)
    plt.plot(np.array(range(1, output_rank_data.shape[0] + 1)), np.asarray(output_rank_data.iloc[:, iteration_layer]), color='b')
    plt.plot(np.array(range(1, input_rank_data.shape[0] + 1)), np.asarray(input_rank_data.iloc[:, iteration_layer]), color='k')
    ...
    plt.plot(np.array(range(1, learning_rate_data.shape[0] + 1)), np.asarray(learning_rate_data.iloc[:, iteration_layer]), color='c')
    plt.plot(np.array(range(1, acc_data.shape[0] + 1)), np.asarray(acc_data), color='g')
    plt.ylabel('Tensor Rank')
    plt.xlabel('Epoch')
    plt.title('Layer '+str(iteration_layer+1))
    plt.gca().legend(('input Rank', 'Output Rank', 'learning rate', 'Test Accuracy'), prop={"size":5})
    plt.grid(True)
    plt.ylim((-.2, 1))
    plt.xlim((0, 200))
...
plt.suptitle('plot title')
for iteration_layer in range(input_rank_data.shape[1]):
    plt.subplot(np.ceil(np.sqrt(input_condition_data.shape[1])), np.ceil(np.sqrt(input_condition_data.shape[1])), iteration_layer+1)
    plt.plot(np.array(range(1, input_condition_data.shape[0] + 1)), np.asarray(input_condition_data.iloc[:, iteration_layer]), color='b')
    plt.plot(np.array(range(1, output_condition_data.shape[0] + 1)), np.asarray(output_condition_data.iloc[:, iteration_layer]), color='k')
    plt.ylabel('Tensor Rank')
    plt.xlabel('Epoch')
    plt.title('Layer '+str(iteration_layer+1))
...
plt.suptitle('plot title')
for iteration_layer in range(input_rank_data.shape[1]):
    ax = plt.subplot(np.ceil(np.sqrt(input_rank_data.shape[1])), np.ceil(np.sqrt(input_rank_data.shape[1])), iteration_layer+1)
    plt.plot(np.array(range(1, input_condition_data.shape[0]+1)), np.asarray(loss_data), color='b')
    plt.ylabel('Loss')
    plt.xlabel('Epoch')
    plt.title('Layer '+str(iteration_layer+1))
    plt.grid(True)
